...
class MultiLayerPerceptron():
    ...
    def optimize_(self, flag):
        ...
        # Compute the gradient explicitl of the loss, this is usefull only in the question 3
        ...
        if flag == "global":
            ...
            self.opt = tf.contrib.opt.ScipyOptimizerInterface(self.loss, method="L-BFGS-B", options={"disp":True})
...
class RBF:
    ...
    def RBF_net(self, omega):
        ...
        # For each datapoint compute the predicted output y
    ...
    def optimize(self, method, omega0):
        ...
        #Call the optimizzation routine
...
def train_RBF(model, omega0):
    ...
    omega = model.optimize(method='L-BFGS-B',omega0=omega0)

    comp_time = time.time() - start

    return omega['x'], omega['nfev'],omega['nit'],comp_time

def run_MLP(X_train, Y_train, X_test, Y_test, N, ro):
    ...
    mlp = MultiLayerPerceptron(num_features=3, N=N, lamda=ro)
    ...
    with tf.Session() as sess:
        ...
        mlp.optimize(sess, X_train_MLP, Y_train)

        #Compute errors and training time
        ...
        print("MLP ---- > objective_value : {} -- MSE test : {} -- time : {} s".format(obj_value, MSE_test, comp_time))
...
def run_RBF(X_train, Y_train, X_test, Y_test, N, ro, num_features, sigma):

    #Initialize the model and the initial value of the parameters

    rbf = RBF(X=X_train, Y=Y_train, sigma=sigma, ro=ro, N=N)

    gen = np.random

    gen.seed(100)

    omega0=gen.rand(N*num_features + N)

    # Run the optimizer

    omega, f_eval, n_iter, comp_time = train_RBF(rbf, omega0)

    # Compute errors and training time

    rbf_test = RBF(X=X_test, Y=Y_test, sigma=sigma, ro=ro, N=N)
    ...
    centers = centers.reshape(N,num_features)

    V = omega[-N:]

    print("RBF ---- > Objective value : {} -- MSE test : {} -- time : {} s".format(obj_value,MSE_test,comp_time))

    #plotter_RBF(V, centers, sigma)

    return MSE_test,obj_value, f_eval, n_iter, comp_time
